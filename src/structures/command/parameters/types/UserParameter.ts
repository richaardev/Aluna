  ...
  return {
    ...
    parse(ctx: CommandContext, argument: string | undefined, opt: UserInterface) {
      ...
      const findMember = argument
        ? ctx.guild?.members.cache.find((m) => m.user.username.toLowerCase().includes(argument!))
        : null;
      ...
      if (options.denySelf && user.id === ctx.author.id)
        throw new Error("Desculpe, mas você não pode mencionar á si mesmo!");
