...
export default class MessageListener {
  ...
  run(message: Message): void {
    ...
    if (message.content.startsWith(`<@!${this.client.user?.id}>`)) {
      ...
      const row = new MessageActionRow().addComponents(
        new MessageButton()
          ...
          .setEmoji("🙋‍♀️")
          .setURL(
            `https://discord.com/api/oauth2/authorize?client_id=${this.client.user?.id}&permissions=0&scope=bot%20applications.commands`,
          ),
      );
    ...
    const command = this.client.commandManager.get(commandName);
    if (!command) return;
