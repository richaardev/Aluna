...
CanvasRenderingContext2D.prototype.drawBlurredImage = function (
  image: Image,
  blur: number,
  imageX: number,
  imageY: number,
...
CanvasRenderingContext2D.prototype.roundImage = function (
  img: Image,
  x: number,
  y: number,
  w: number,
...
CanvasRenderingContext2D.prototype.roundImageCanvas = (
  img: Image,
  w = img.width,
  h = img.height,
  r = w * 0.5,
...
String.prototype.abbreviate = function (precision = 2) {
  const _this = parseInt(this as string);
  ...
  return _this < 1000
    ? "" + _this
    : (calc.indexOf(".") === calc.length - 3 ? calc.replace(/\.00/, "") : calc) + suffsFromZeros[keyTyped];
};
